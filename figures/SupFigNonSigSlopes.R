...
# Read the quantiles of non-significant slopes
...
p_nonSig <- ggplot(data=datNonSigSlopes_long,aes(x=age,y=value,group=variable,color=CItype)) + geom_line() +  theme_bw()  + #+ geom_ribbon(aes(ymin=lIQR,ymax=hIQR),colour = NA) 
    ...
    xlab("Age") + ylab("Effect (log HR)\n(non-significant variants)") + scale_x_continuous(breaks=seq(41,55,2)) + ylim(c(-0.05,0.05))
...
ggsave(file="/nfs/scistore13/robingrp/sojavee/figures/SupFig_NonSigSlopes.pdf",plot=p_nonSig,width=6,height=3.5)
